...
[Stage 3: Demo 开发] → 5-7 个可运行项目
    ↓
[Stage 5: Pitch Deck] → pitch-script.md + pitch-deck.html per project (--skip-pitch 时跳过)
    ↓ (pitch files copied into demo/ dirs)
[Stage 4: GitHub 发布] → 5-7 个 GitHub 仓库 (含 pitch 产出, --skip-publish 时跳过)
```
