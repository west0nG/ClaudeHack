...
**实现决策变更**：与阶段一 Research session 的实现方式一致，阶段二采用**单 coordinator prompt** 模式（`prompts/stage2/prd.md`），而非为每个角色创建独立的 prompt 文件。每张 Idea Card 对应 1 个 Claude Code session，session 内部通过 **Agent tool** 按步骤调度 5 个角色 sub-agent。

> **变更理由**：让 Claude Code session 自己管理 sub-agent 生命周期更自然——它能在内部协调 Product ↔ Technical 反馈循环、Critic 打回机制、Pitch 微调，共享文件上下文（product-design.md, technical-review.md 等中间文件），减少中控脚本的复杂度。

**中控编排（`control/stages/stage2.py`）**：
- 读取 `prompts/stage2/prd.md` 模板，用 `{{idea_card_content}}` 和 `{{theme}}` 渲染
- 每张 Idea Card 一个 `SessionConfig`：session_id=`prd-{slug}`, timeout=1800s, budget=$5
- `session_mgr.run_many()` 并行执行（Semaphore 控制）
- 成功: 收集 `prd.md` + `wireframe.html` → 复制到 `workspace/stage2/output/`
- 淘汰: 检查 `ELIMINATED.md` → 发送 `prd_eliminated` 事件
- 新增 `--idea-card` 调试入口：跳过 Stage 0+1，直接运行 Stage 2
...
**实现决策变更**：与阶段二的实现方式一致，阶段三采用**单 coordinator prompt** 模式（`prompts/stage3/dev.md`），而非为每个角色创建独立的 prompt 文件。session 内部通过 Claude Code 的 **Agent tool** 管理各 sub-agent。

> **变更理由**：与 Stage 2 的 `prd.md` 模式保持一致——让 Claude Code session 自己管理 sub-agent 生命周期更自然，能在内部协调循环、共享文件上下文、按 wave 并行调度屏幕任务，减少中控脚本的复杂度。

**7 步流水线（在单个 coordinator prompt 内编排）**：

| 步骤 | 角色 | 形式 | 对应设计文档角色 |
|------|------|------|-----------------|
| Step 1 | Planner Agent | Agent tool sub-agent | 9.3.1 Planner Sub-agent |
| Step 2 | Scaffold | coordinator 直接用 Bash tool | 9.3.2 Orchestrator 骨架搭建部分 |
| Step 3 | Coding Agents (per screen, wave-parallel) | Agent tool sub-agents | 9.3.3 Coding Sub-agents |
| Step 4 | Designer Agent | Agent tool sub-agent | 9.3.4 Designer Agent |
| Step 5 | Reviewer Agent | Agent tool sub-agent | 9.3.5 Reviewer Agent |
| Step 6 | Fix Issues (conditional) | Agent tool sub-agents | 9.4 中间层/最外层循环 |
| Step 7 | Final Verification | coordinator 直接执行 | 最终检查 |

**循环机制映射**：

| 设计文档循环层级 | 实际实现 |
|-----------------|---------|
| 最内层：Coding Agent 自修复 (3-5次) | Step 3: 每个 Coding Agent 内部 `npm run build` + fix，上限 5 次 |
| 中间层：Reviewer ↔ Coding (2-3次) | Step 5 + Step 6: Reviewer 发现 ISSUES_FOUND → 启动 Fix Agents → 重跑 Reviewer，上限 2 cycles |
| 最外层：集成级 | Step 5 的 Reviewer 执行端到端验证（路由、数据流、wow moment）|

**新增机制（设计文档未涉及）**：

- **`BUILD_FAILED.md` 失败标志**：类比阶段二的 `ELIMINATED.md`，当项目最终无法构建时写入此文件，中控据此判断 session 成败
- **`README.md` 自动生成**：构建成功后写入快速启动指南
- **`--prd` CLI 调试入口**：跳过 Stage 0+1+2，直接对单个 PRD 文件运行 Stage 3（同 `--idea-card` 模式）
...
| 参数 | 值 | 理由 |
|------|-----|------|
| `timeout_seconds` | 3600 (1 小时) | Demo 开发含多轮 build+fix，需要远多于 PRD 的时间 |
| `max_budget_usd` | 10.0 | 开发任务 token 消耗显著高于 PRD ($5) |
| `allowed_tools` | Bash, Agent, Read, Write, Glob, Grep | 新增 Bash（npm 命令），移除 WebSearch/WebFetch（开发阶段不需要搜索）|
| `model` | sonnet | 与其他阶段一致 |

**成功判定逻辑（中控 `_collect_project_outputs()`）**：
...
10. Stage 2: 为每张 Idea Card 启动 PRD session → 收集 prd-*.md 到 workspace/stage2/output/
11. Stage 3: 为每份 PRD 启动 Dev session → 收集项目目录 (检查 package.json 或 BUILD_FAILED.md)
...
│   │   └── dedup.md             # 去重 + 质量过滤
│   ├── stage2/                  # ✅ 已实现 (单 coordinator prompt 模式)
│   │   └── prd.md               # 实际: 单 prompt 内嵌 5 角色 sub-agent 指令
│   │   # 原设计: product.md, technical.md, critic.md, pitch.md, wireframe.md
│   │   # 变更: 合并为单文件，session 内部通过 Agent tool 管理
│   └── stage3/                  # ✅ 已实现 (单 coordinator prompt 模式)
│       └── dev.md               # 实际: 单 prompt 内嵌 7 步流水线指令
│       # 原设计: planner.md, orchestrator.md, coding.md, designer.md, reviewer.md
│       # 变更: 合并为单文件，session 内部通过 Agent tool 管理
│
├── templates/
...
│           └── demo/            # 产出项目 (package.json, src/, README.md)
│
├── .venv/                       # Python 虚拟环境
├── requirements.txt
├── .gitignore
...
**实现说明**：阶段二和阶段三实际采用**单 coordinator prompt** 模式。上表中阶段二的 5 个角色和阶段三的 5 个角色分别在 `prd.md` 和 `dev.md` 内作为 sub-agent 指令嵌入，由 coordinator 按步骤调度。形式上仍然是 Agent tool sub-agent，但 prompt 组织为单文件而非多文件。
...
### Stage 2 & Stage 3 实现决策

| 决策 | 选择 | 理由 |
|------|------|------|
| Prompt 组织模式 | 单 coordinator prompt (非多文件) | session 内部通过 Agent tool 管理 sub-agents，与 Stage 1 research.md 模式一致，减少中控复杂度 |
| Stage 2 prompt | `prompts/stage2/prd.md` 内嵌 5 角色 | coordinator 按步骤调度 Product→Technical→Critic→Pitch→Wireframe |
| Stage 3 prompt | `prompts/stage3/dev.md` 内嵌 7 步 | coordinator 管理完整开发流水线 |
| Stage 3 Scaffold 执行方式 | coordinator 直接用 Bash，不 spawn sub-agent | 脚手架初始化是确定性操作（npm create vite, npm install），不需要 AI 判断 |
| Stage 3 超时 | 3600s (1 小时) | Demo 开发含多轮 build+fix+多屏幕实现，远超 PRD 生成 |
| Stage 3 预算 | $10/session | 开发任务 token 消耗高于 PRD ($5)、Research ($2) |
| Stage 3 allowed_tools | 新增 Bash，移除 WebSearch/WebFetch | 开发阶段需要 npm 命令，不需要搜索 |
...
| 主题提取 | `_extract_theme_from_prd()` | `--prd` 模式下从 PRD 内容推断主题 |
